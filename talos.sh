...
if ls /etc/pve/nodes/*/qemu-server/$VMTID.conf 1> /dev/null 2>&1; then
    echo ${RESET}${BOLD}${RED}
    echo "Existing configuration located, please try again with a" | pv -qL 25
    echo "new ID or remove the existing file which is located: " | pv -qL 25
    echo ${RESET}${BOLD}
...
sleep 2
qm create $VMTID --name "talos-1.10.6" --ostype l26 --cpu host --cores 2 \
--memory 2048 --net0 virtio,bridge=vmbr0,tag=15 --scsihw virtio-scsi-pci
qm set $VMTID --scsi0 thin1:0,import-from=/mnt/pve/cephfs/template/iso/talos-nocloud-amd64.iso
